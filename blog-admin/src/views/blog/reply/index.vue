...
<script>
import { findAll, create, updata, del, batchDel } from "@/api/reply.js";
export default {
  data() {
    return {
      search: {
          ...
          if (code == "200") {
            let new_list = data.rows.map((el, index) => {
              return {
                ...
                comment: el.comment,
                user: el.user,
                replyUserName: el.user.username,
                replyUserLogo: el.user.avatar,
                replyUserEmail: el.user.email,
                replyBlog: el.article.title,
